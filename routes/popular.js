...
router.get("/", async (req, res) => {
    try {
        const popularbook = await reviewModel.find({rating:{$gte:4}})
        .populate("bookId");
       ...
       const popularcategories = [];
        for(let i=0;i<popularbook.length;i++)
         {
            const popularauthor = await authorModel.findById({_id:popularbook[i].bookId.authorId});
            popularauthors.push(popularauthor);
            const popularcategory = await categoryModel.findById({_id:popularbook[i].bookId.categoryId});
